...
async def get_cities_kb(separated_cities_list: list) -> list:
    ...
    for i, chunk in enumerate(separated_cities_list):
        ...
        if (
            len(chunk) == env_parameters.NUMBER_OF_CITIES_PER_PAGE
        ):  # chunk - channels per page
        ...
        for city in range(0, end_len):
            kb.button(
                text=chunk[city]["city_name"], callback_data=chunk[city]["city_id"]
            )  # values from fsm_functions
            kb.adjust(1)

        if len(separated_cities_list) > 1:
            kb.row(
                InlineKeyboardButton(text=await _("BACK"), callback_data="back"),
